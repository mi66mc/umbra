...
Missing keys, malformed snapshots, unsupported versions, and authentication failures fail closed without erasing the encrypted artifact. Atomic replacement preserves the prior snapshot if a write cannot be promoted. Legacy plaintext caches are intentionally not auto-deleted because the user may need to recover them; operators must treat them as sensitive, back them up offline, remove them deliberately, and sync again.
