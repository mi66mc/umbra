...
The distribution rule is one envelope per active intended device for every creation, bootstrap approval, member grant, invite acceptance, and rotation. Legacy user-scoped writes and unlock fallbacks fail closed. Protocol-v3 checkpoints commit sorted recipient metadata and hashes of opaque envelope bytes: a sync response carries the ciphertext only for the authenticated device, while redacted metadata lets every trusted device calculate the same commitment without receiving another device's envelope.
