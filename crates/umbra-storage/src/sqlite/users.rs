...
use crate::{
    ...
    sqlite::convert::{user_auth_from_row, user_from_row},
};
...
impl SqliteStorage {
    ...
    pub async fn find_user_by_email(&self, email: &str) -> Result<UserRecord, StorageError> {
        let row = sqlx::query(
            r#"
            SELECT id, email, display_name, public_key, encrypted_private_key,
                   created_at, updated_at, disabled_at
            FROM users
            WHERE email = ?1
            "#,
    ...
    pub async fn find_user_by_id(&self, user_id: uuid::Uuid) -> Result<UserRecord, StorageError> {
        let row = sqlx::query(
            r#"
            SELECT id, email, display_name, public_key, encrypted_private_key,
                   created_at, updated_at, disabled_at
    ...
    pub async fn upsert_user_auth(
        ...
        .bind(input.auth_data.to_string())
        .fetch_one(&self.pool)
        .await
        .map_err(map_sqlx_error)?;
