use umbra_core::{OrgId, UserId};
use uuid::Uuid;

use crate::convert::{member_state_to_str, org_role_to_str};
use crate::error::map_sqlx_error;
use crate::sqlite::SqliteStorage;
use crate::sqlite::convert::{org_from_row, org_member_from_row};
use crate::{CreateOrg, OrgMemberRecord, OrgRecord, StorageError, UpsertOrgMember};
...
impl SqliteStorage {
    ...
    pub async fn find_org_by_id(&self, org_id: OrgId) -> Result<OrgRecord, StorageError> {
        let row = sqlx::query(
            "SELECT id, name, created_by, created_at, updated_at, deleted_at FROM orgs WHERE id = ?1",
        )
    ...
    pub async fn list_org_members(
        ...
        let rows = sqlx::query(
            "SELECT org_id, user_id, role, state, created_at, updated_at FROM org_members WHERE org_id = ?1 ORDER BY created_at ASC",
        )
        .bind(org_id.to_string())
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn find_org_member(
        ...
        let row = sqlx::query(
            "SELECT org_id, user_id, role, state, created_at, updated_at FROM org_members WHERE org_id = ?1 AND user_id = ?2",
        )
        .bind(org_id.to_string())
        .bind(user_id.to_string())
        .fetch_optional(&self.pool)
